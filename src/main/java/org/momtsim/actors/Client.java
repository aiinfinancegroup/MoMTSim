...
import org.momtsim.base.ClientProfile;
...
import org.momtsim.parameters.BalancesClients;
...
public class Client extends SuperActor implements HasClientIdentity, Identifiable, Steppable {
    ...
    Client(MoMTSimState state, ClientIdentity identity) {
        super(state);
    ...
    public Client(MoMTSimState state) {
    ...
    public void step(SimState state) {
        ...
        int stepTargetCount = momtSim.getStepTargetCount();
        if (stepTargetCount > 0) {
            MersenneTwisterFast random = momtSim.random;
            int step = (int) state.schedule.getSteps();
            Map<String, Double> stepActionProfile = momtSim.getStepProbabilities();

            int count = pickCount(random, stepTargetCount);

            for (int t = 0; t < count; t++) {
                String action = pickAction(random, stepActionProfile);
                StepActionProfile stepAmountProfile = momtSim.getStepAction(action);
                double amount = pickAmount(random, action, stepAmountProfile);

                List<Transaction> transactions = makeTransaction(momtSim, step, action, amount);
                if (!momtSim.onTransactions(transactions)) {
     ...
     *
     * @param state reference to the MoMTSimState instance driving the simulation
     * @return the selected Merchant
     */
    private Merchant pickMerchant(MoMTSimState state) {
     ...
     */
    private Client pickClient(MoMTSimState state) {
    ...
    private List<Transaction> makeTransaction(MoMTSimState state, int step, String action, double amount) {
    ...
    protected Transaction handleCashOut(MoMTSimState state, int step, double amount) {
    ...
    protected Transaction handleDebit(MoMTSimState state, int step, double amount) {
        double oldBalanceOrig = this.getBalance();
